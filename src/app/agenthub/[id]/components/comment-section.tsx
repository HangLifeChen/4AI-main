...
import { createAgentComments, getAgentComments, getCommentDetail, likeAgentComments, unlikeAgentComments } from '../../request';
...
const CommentSection: React.FC<CommentSectionProps> = ({ comment, className, respositoryId }) => {
  ...
  const handleLikeMethod = async (id: number, isLike: boolean) => {
    authLoginStatus();
    const requestMethod = isLike ? unlikeAgentComments : likeAgentComments
  ...
  const handleReply = async () => {
    authLoginStatus();
    const res = await createAgentComments({
      repo_id: respositoryId,
        ...
        <div className="flex gap-3 items-start">
          ...
          <div className="flex-1 flex flex-col gap-3">
            <div className="flex items-center">
              <span className="font-semibold">{displayComment.username}</span>
