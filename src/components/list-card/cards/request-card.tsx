...
const RequestCard = ({ content, isSelected, onClick, ondoubleClick, isRankList = false }: IProps) => {

  const router = useRouter()

  const jumpToUserCenter = (id: string) => {
  ...
  const combineClick = () => {
    if (isRankList) {
      ...
      router.push(`/requests/${content.id}`)
    }
  }
      ...
      <div
        className={
          cn(
            ...
            isSelected && "bg-gradient-to-r from-[#825D00] to-[#643800]",
      ...
      >
        ...
        <div className="flex justify-between">
          <div className="flex items-center">
            <div className="flex items-center gap-2" onClick={(e) => { e.stopPropagation(); jumpToUserCenter(content.create_by) }}>
              <Avatar
                width={20}
                height={20}
